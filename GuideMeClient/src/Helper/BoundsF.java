...
public class BoundsF {
    ...
    public boolean intersects(PointF intersectionPoint) {
        
        if( intersectionPoint.getX() <=x1 &&  intersectionPoint.getX() >=x2 && intersectionPoint.getY() >=y1 && intersectionPoint.getY() <=y2  )
            return true;
    ...
    public BoundsF(byte[] pBytes){
        ...
        public BoundsF(byte[] content,int start, int end){
       
        
        this.x1 = byteConverter.fromArrayByte2Float(content,start+0,4);
        this.y1 = byteConverter.fromArrayByte2Float(content,start+4,4);
        this.x2 = byteConverter.fromArrayByte2Float(content,start+8,4);
        this.y2 = byteConverter.fromArrayByte2Float(content,start+12,4);
    }
